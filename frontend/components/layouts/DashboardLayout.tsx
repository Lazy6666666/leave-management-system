...
export default function DashboardLayout({
  ...
  const NavItemComponent = ({ item, onClick, collapsed = false }: { item: NavItem, onClick?: () => void, collapsed?: boolean }) => {
    ...
    return (
      ...
      >
        <item.icon className={`h-5 w-5 flex-shrink-0 transition-all duration-200 ${!isActive && 'group-hover:scale-110 group-hover:rotate-3'}`} />
        <span className={`transition-all duration-300 ${collapsed ? 'opacity-0 w-0 overflow-hidden' : 'opacity-100'}`}>{item.name}</span>
      </Link>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className={`hidden lg:fixed lg:inset-y-0 lg:left-0 lg:z-50 lg:block lg:bg-card lg:border-r transition-all duration-300 ease-in-out ${sidebarCollapsed ? 'lg:w-16' : 'lg:w-60'}`}>
        <div className="flex h-full flex-col">
          ...
          <div className="flex h-16 items-center px-6 border-b">
            <Link 
              href="/dashboard" 
              className="flex items-center gap-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 rounded-md transition-transform duration-200 hover:scale-105"
            >
              <Calendar className="h-6 w-6 text-primary flex-shrink-0 transition-transform duration-200 hover:rotate-12" />
              <h1 className={`text-xl font-bold transition-all duration-300 ${sidebarCollapsed ? 'opacity-0 w-0' : 'opacity-100'}`}>Leave Portal</h1>
            </Link>
          </div>
          ...
          <div className="flex h-full flex-col">
            ...
            <SheetHeader className="border-b p-6">
              <SheetTitle className="flex items-center gap-2 text-left">
                ...
                <span className="text-xl font-bold">Leave Portal</span>
              </SheetTitle>
            </SheetHeader>
      ...
      <div className={`transition-all duration-300 ease-in-out ${sidebarCollapsed ? 'lg:pl-16' : 'lg:pl-60'}`}>
        {/* Top bar - Consistent height: 64px, responsive padding */}
        ...
        <main id="main-content" className="py-4 md:py-6 lg:py-8" role="main" aria-label="Main content">
